function robotdraw_extcontroller(chain,external_controller,varargin)
%
...
addParameter(ps,'PRINT_JOINT_POS',0);
...
addParameter(ps,'PLOT_ENDEFFECTOR',false);          % plot EE link
addParameter(ps,'PLOT_EETRAJ',false);               % plot EE trajectory
...
PLOT_ENDEFFECTOR     = ps.Results.PLOT_ENDEFFECTOR;
PLOT_EETRAJ     = ps.Results.PLOT_EETRAJ;
...
endeff_p=chain.joint(chain.n_joint).p;
endeff_R=chain.joint(chain.n_joint).R;
...
while true % loop
    % Update
    tick = tick + 1;
    T_endeff=pr2t(endeff_p,endeff_R);
    [q_rev, do_draw]=external_controller(tick, q_rev,T_endeff);
    ...
    %Update the end effector transform
    endeff_p=chain.joint(chain.n_joint).p;
    endeff_R=chain.joint(chain.n_joint).R;
    T_endeff=pr2t(endeff_p,endeff_R);

    if PLOT_ENDEFFECTOR      %Plot End Effector Frame
      plot_T(T_endeff,'fig_idx',1,'subfig_idx',1,...
          'PLOT_AXIS',1,'all',0.5,'alc','','alw',2,'als','-',...
          'PLOT_AXIS_TIP',0,'atr',0.05,...
          'PLOT_SPHERE',0,'sr',0.05,'sfc','m','sfa',0.5,...
          'text_str','$EE$','text_fs',20,'text_color','k','text_interp','latex',...
          'USE_ZOOMRATE',1);
      fprintf('End effector pos: %.3f %.3f %.3f\n',...
        endeff_p(1),endeff_p(2),endeff_p(3));
      endeff_R
    end

    if do_draw > 0
      eetraj(size(eetraj,1)+1,:)=endeff_p;
      plot_traj(eetraj,'tls','--');
    else
      eetraj=[];
    end
    ...
    plot_title_with_text(title_str,'fig_idx',1,'tfs',20,'tfc',tfc,'interpreter','latex');
...
fprintf('[robot animation with external controller] Done.\n');
